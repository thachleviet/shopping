...
@section('content')
    <section class="content-header">
        <h1>
            Dashboard
            <small> <a href="{{asset(route('permissions'))}}"> <i class="fa fa-angle-double-right"></i>Danh mục vai trò</a></small>
                        ...
                        <div class="box-body">
                            <div class="form-group row {{($errors->has('name')) ? 'has-error': ''}}" >
                                @foreach ($permissions as $permission)
                                    <div class="col-sm-4">  <input type="checkbox"  name="permission[]" value="{{$permission->id}}" > <label >{{$permission->name}}</label></div>
                                @endforeach
                            </div>
                        </div>
